...
public class QuantitySetter(IQuantityComparatorMap quantityComparatorMap) : IQuantitySetter
{
  ...
  private IList<IndexQuantity> ProcessDynamicPrimitive(DynamicPrimitive dynamicPrimitive)
  {
    ...
    {
      case Hl7.Fhir.ElementModel.Types.Quantity quantity:
        return SetFPQuantity(quantity);
      case decimal d:
        return new List<IndexQuantity>() { SetIndexQuantity(d, null, null, null, null) };
      default:
  ...
  private IList<IndexQuantity> SetFPQuantity(Hl7.Fhir.ElementModel.Types.Quantity quantity)
  {
    var resourceIndex = SetIndexQuantity(quantity.Value, null, null, null, quantity.Unit);
    return new List<IndexQuantity>() { resourceIndex };
  }
