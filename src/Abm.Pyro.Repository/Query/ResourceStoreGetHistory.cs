...
public class ResourceStoreGetHistory(
  PyroDbContext context,
  IPaginationSupport paginationSupport) : IResourceStoreGetHistory
{
  public async Task<ResourceStoreSearchOutcome> Get(SearchQueryServiceOutcome searchQueryServiceOutcome)
  {
    var query = context.Set<ResourceStore>().Where(x => true);
